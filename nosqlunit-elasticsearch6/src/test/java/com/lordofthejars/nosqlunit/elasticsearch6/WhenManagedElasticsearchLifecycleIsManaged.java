...
public class WhenManagedElasticsearchLifecycleIsManaged {
	private static final String DEFAULT_ELASTICSEARCH_HOME = "/opt/elasticsearch-6.7.1/";
	...
	public void managed_elasticsearch_should_be_started_from_elasticsearch_home() throws Throwable {
		...
		when(lowLevelElasticSearchOperations.assertThatConnectionToElasticsearchIsPossible(any(Settings.class),
		...
		when(commandLineExecutor.startProcessInDirectoryAndArguments(anyString(), anyListOf(String.class)))
		...
		try {
			...
			managedElasticsearch.managedElasticsearchLifecycleManager
					.setOperatingSystemResolver(operatingSystemResolver);
			managedElasticsearch.managedElasticsearchLifecycleManager
					.setLowLevelElasticSearchOperations(lowLevelElasticSearchOperations);

			managedElasticsearch.before();
			managedElasticsearch.after();

			verify(commandLineExecutor).startProcessInDirectoryAndArguments(
					ManagedElasticsearchLifecycleManager.DEFAULT_ELASTICSEARCH_TARGET_PATH, getExpectedXCommand());
		} finally {
			if (oldElasticsearchHome == null) {
	...
	public void managed_elasticsearch_should_be_started_from_windows_custom_location() throws Throwable {
		...
		verify(commandLineExecutor).startProcessInDirectoryAndArguments(
				ManagedElasticsearchLifecycleManager.DEFAULT_ELASTICSEARCH_TARGET_PATH, getExpectedWindowsCommand());
	...
	public void managed_elasticsearch_should_be_stopped() throws Throwable {
		...
		assertThat(ConnectionManagement.getInstance().isConnectionRegistered("127.0.0.1",
				ManagedElasticsearchLifecycleManager.DEFAULT_PORT), is(false));
	}
